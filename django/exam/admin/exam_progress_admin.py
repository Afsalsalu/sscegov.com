...
from exam.models import MainExamProgress, TecExamProgress
...
@admin.register(TecExamProgress)
class TecExamProgressAdmin(admin.ModelAdmin):
    list_display = (
        "user",
        "exam_category",
        "status",
    ...
    list_filter = ("status", "exam_category", "user")  # Filter by status, category, and user
    search_fields = ("user__username", "exam_category__name")  # Search by username and category
    readonly_fields = ("started_at", "completed_at")
    list_per_page = 20
    ordering = ("-started_at",)
